...
class CommonClient {
  ...
  async selectByVisibleText(selector, text) {
    await page.evaluate((selector, text) => {
      let selectID = document.getElementById(selector.split('#')[1]);
      let indexElement = [...selectID.options].findIndex(option => option.text === text);
      let valueElement = [...selectID.options][indexElement].value;
      console.log(valueElement);
      return valueElement;
    }, selector, text).then(async (valueElement) => {
      await page.select(selector, valueElement);
    });
  }

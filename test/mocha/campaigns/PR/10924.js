...
scenario('PR-10924: Check that the images is well displayed in the edit combination', () => {
  ...
  scenario('Add a new value for "Color" attribute', client => {
    ...
    test('should search click on "View" button', () => client.waitForAndClick(Attributes.view_button, 1000));
    test('should search click on "Add new value" button', () => client.waitForAndClick(Attributes.add_new_value_button, 1000));
    ...
    test('should get the "Id" of the new created value', async () => {
      await client.waitForAndClick(Attributes.view_button,);
      await client.waitForAndClick(Attributes.sort_by_position.replace('%D', 'down'), 1000);
      await client.getTextInVar(Attributes.id_product.replace('%D', 1), 'idProduct', 2000);
    });
  }, 'common_client');
  scenario('Check that the new created value is in the list of the attributes in the product page', client => {
    ...
    test('should Check that the new created value is in the list of the colors', () => client.isExisting(AddProduct.Combination.attribute_color_label.replace('%D', tab['idProduct'])));
  }, 'common_client');

...
scenario('This scenario is based on the bug described in this PR: https://github.com/PrestaShop/PrestaShop/pull/9466', () => {
  ...
  scenario('Create a new product in the Back Office', client => {
    ...
    test('should set the "Quantity" input', () => client.clearInputAndSetValue(AddProductPage.Basic_settings.quantity_input, productData.quantity));
    ...
    test('should close the symfony toolbar', async () => {
      await client.isVisible(AddProductPage.close_symfony_toolbar, 7000);
      if (visible) {
        await client.waitForAndClick(AddProductPage.close_symfony_toolbar);
      }
    });
    test('should click on "Online"', () => client.waitForAndClick(AddProductPage.online_switcher));
    scenario('Add Attribute', client => {
      ...
      test('should choose the combinations', async () => {
        await client.waitForAndClick(AddProductPage.Combination.attribute_size_checkbox_button.replace('%ID', 1), 1000);
        await client.waitForAndClick(AddProductPage.Combination.attribute_size_checkbox_button.replace('%ID', 2), 1000);
      });
      test('should click on "Generate" button', async () => {
        await client.checkBrowserMessage('');
        await client.waitForAndClick(AddProductPage.Combination.generate_combination_button, 3000);
      });
    ...
    test('should click on "Save" button', () => client.waitForAndClick(AddProductPage.save_button, 3000));

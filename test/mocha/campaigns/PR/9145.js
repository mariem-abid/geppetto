...
scenario('This scenario is based on the bugs described on this PR: https://github.com/PrestaShop/PrestaShop/pull/9145', () => {
  ...
  scenario('This scenario is based on the bug described on this BOOM: https://forge.prestashop.com/browse/BOOM-3495', () => {
    ...
    scenario('Check that quantities are well saved', client => {
      ...
      test('should check that the first combination quantity is equal to "' + productData.combinationsQuantities.firstQuantity + '"', () => client.checkAttributeValue(AddProductPage.Combination.attribute_quantity_input.replace("%NUMBER", 1), 'value', productData.combinationsQuantities.firstQuantity, 4000));
      test('should check that the second combination quantity is equal to "' + productData.combinationsQuantities.secondQuantity + '"', () => client.checkAttributeValue(AddProductPage.Combination.attribute_quantity_input.replace("%NUMBER", 2), 'value', productData.combinationsQuantities.secondQuantity, 4000));
    }, 'common_client');

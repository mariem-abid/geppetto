...
module.exports = {
  ...
  async createProduct(productData) {
    scenario('Create a new product in the Back Office', client => {
      ...
      test('should check and close the green validation', () => client.waitForAndClick(AddProduct.close_validation_button, 3000, {visible: true}));
      ...
      if (productData.hasOwnProperty('pricing')) {
        scenario('Edit product pricing', client => {
          test('should click on "Pricing"', () => client.waitForAndClick(AddProduct.Pricing.product_pricing_tab));
          test('should set the "Price per unit (tax excl.)"', () => client.waitForAndSetValue(AddProduct.Pricing.unit_price, productData['pricing']['unitPrice']));
          test('should set the "Unit"', () => client.waitForAndSetValue(AddProduct.Pricing.unity, productData['pricing']['unity']));
          test('should set the "Price (tax excl.)"', () => client.waitForAndSetValue(AddProduct.Pricing.pricing_wholesale, productData['pricing']['wholesale']));
          test('should click on "Add specific price" button', () => client.waitForAndClick(AddProduct.Pricing.pricing_add_specific_price_button));
          test('should change the reduction type to "Percentage"', async () => {
            await client.waitFor(3000);
            await client.waitForAndSelect(AddProduct.Pricing.specific_price_reduction_type_select, productData['pricing']['type']);
          });
          test('should set the "Discount" input', () => client.waitForAndSetValue(AddProduct.Pricing.specific_price_discount_input, productData['pricing']['discount']));
          test('should click on "Apply" button', () => client.waitForAndClick(AddProduct.Pricing.specific_price_save_button));
        }, 'catalog/product');
      }
      ...
      scenario('Save the product then close the green validation', client => {
        ...
        test('should check and close the green validation', async () => {
          await client.checkTextValue(AddProduct.validation_msg, 'Settings updated.');
          await client.waitForAndClick(AddProduct.close_validation_button, 3000 , {visibile: true});
        });
      }, 'common_client');
    }, 'common_client');
  },
